...
QDirect3D11Widget::QDirect3D11Widget(QWidget* parent)
	...
	, m_pDepthStencilView(nullptr)  // ✅ 이게 핵심!
...
bool QDirect3D11Widget::init()
{
	...
	if (!meshSceneDepth) {
		...
		std::cout << "VolumeToTexture object created at: " << meshSceneDepth->m_quadVertexBuffer << std::endl;
	}
	...
	DXCall(m_pDevice->CreateRenderTargetView(deltaZTexRtv, nullptr, &m_volumeRTV));*/

	CreateScaleFitResources(m_pDevice,
		width(),
		height(),
		DXGI_FORMAT_R32_FLOAT,
		scaleRes);
...
//	fileReader->floatData.resize(size_t(w) * h * d);
	...
	std::cout << "floatData check: "
		<< fileReader->floatData[0] << " "
		<< fileReader->floatData[100] << " "
		<< fileReader->floatData[10000] << std::endl;

	if (!ok) {
		OutputDebugStringA(" NormalizeVolumeU16 failed\n");
		return;
	}
	if (fileReader->floatData.size() < size_t(w) * h * d) {
...
void QDirect3D11Widget::FullScreenPassSet()
{
	...
	//// SceneDepth 텍스처가 실제로 데이터를 가지고 있는지 확인
	//D3D11_TEXTURE2D_DESC desc;


	//m_depthTexture->GetDesc(&desc);
	//qDebug() << "SceneDepth format:" << desc.Format;
	//qDebug() << "SceneDepth size:" << desc.Width << "x" << desc.Height;
...
void QDirect3D11Widget::RenderMeshWithDepthPeeling(ID3D11DeviceContext* context)
{
	...
	for (int layer{}; layer < MAX_DEPTH_PEELS; ++layer)
	{
		...
		depthDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO; //  중요!
...
void QDirect3D11Widget::CreateDepthStencil()  // 또는 initializeGL 안에서
{
	...
	D3D11_DEPTH_STENCIL_VIEW_DESC dsvDesc = {};
	dsvDesc.Format = DXGI_FORMAT_D32_FLOAT;  //  명시
	dsvDesc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D;
	dsvDesc.Texture2D.MipSlice = 0;
	...
	qDebug() << "Depth buffer created with SRV support";
}
	...
	for (int i{}; i < 4; ++i) {
		...
		if (0 == i) {
			...
			if (fileReader) {
				...
				for (int i{}; i < fileReader->m_height; ++i)
				{
...
ID3D11Texture2D* QDirect3D11Widget::CreateTexture2DUAV(
	ID3D11Device* device,
	UINT width,
	UINT height,
	DXGI_FORMAT format,
...
)
{
	...
	*outUAV = nullptr;
	...
	desc.MipLevels = 1;
	desc.ArraySize = 1;
	desc.Format = format; // DXGI_FORMAT_R32_FLOAT 권장
	desc.SampleDesc.Count = 1;
	desc.Usage = D3D11_USAGE_DEFAULT;
	...
	hr = device->CreateUnorderedAccessView(texture, &uavDesc, outUAV);
	if (FAILED(hr))
	{
		texture->Release();
		return nullptr;
...
bool QDirect3D11Widget::CreateScaleFitResources(
	ID3D11Device* device,
	UINT width,
	UINT height,
	DXGI_FORMAT format,
...
{
	std::cout << "0번 CreateScaleFitResources" << std::endl;
	...
	texDesc.MipLevels = 1;
	texDesc.ArraySize = 1;
	texDesc.Format = format; // DXGI_FORMAT_R32_FLOAT 권장
	texDesc.SampleDesc.Count = 1;
	texDesc.Usage = D3D11_USAGE_DEFAULT;
	...
	HRESULT hr = device->CreateTexture2D(&texDesc, nullptr, &outRes.deltaZTex);
	if (FAILED(hr))
	{
		_com_error err(hr);
		LPCTSTR errMsg = err.ErrorMessage();

		std::cout << "1번 CreateTexture2D" << std::endl;
		OutputDebugString(errMsg); // 디버그 출력창에 메시지 표시
	...
	hr = device->CreateUnorderedAccessView(outRes.deltaZTex, &uavDesc, &outRes.deltaZUAV);
	if (FAILED(hr))
	{
		_com_error err(hr);
		LPCTSTR errMsg = err.ErrorMessage();

		std::cout << "2번 CreateUnorderedAccessView" << std::endl;
		OutputDebugString(errMsg); // 디버그 출력창에 메시지 표시
	...
	hr = device->CreateShaderResourceView(outRes.deltaZTex, &srvDesc, &outRes.deltaZSRV);
	if (FAILED(hr))
	{
		_com_error err(hr);
		LPCTSTR errMsg = err.ErrorMessage();

		std::cout << "3번 CreateShaderResourceView" << std::endl;
		OutputDebugString(errMsg); // 디버그 출력창에 메시지 표시

		return false;
	}

	// 4. Staging 텍스처 생성 (CPU 읽기용)
	D3D11_TEXTURE2D_DESC stagingDesc = texDesc;
	outRes.deltaZTex->GetDesc(&stagingDesc);
	stagingDesc.Usage = D3D11_USAGE_STAGING;
	stagingDesc.BindFlags = 0;
	stagingDesc.CPUAccessFlags = D3D11_CPU_ACCESS_READ;
	stagingDesc.MiscFlags = 0;

	hr = device->CreateTexture2D(&stagingDesc, nullptr, &outRes.stagingTex);
	if (FAILED(hr))
	{
		_com_error err(hr);
		LPCTSTR errMsg = err.ErrorMessage();

		std::cout << "4번 CreateTexture2D" << std::endl;
		OutputDebugString(errMsg); // 디버그 출력창에 메시지 표시

		return false;
	}

	// 5. 상수 버퍼 생성 (GPU에 다시 넘길 값 저장용)
	D3D11_BUFFER_DESC cbDesc = {};
	cbDesc.ByteWidth = sizeof(ScaleConstants);
	cbDesc.Usage = D3D11_USAGE_DEFAULT;
	cbDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	cbDesc.CPUAccessFlags = 0;
	cbDesc.MiscFlags = 0;

	hr = device->CreateBuffer(&cbDesc, nullptr, &outRes.constantBuffer);
	if (FAILED(hr))
	{
		_com_error err(hr);
		LPCTSTR errMsg = err.ErrorMessage();
		...
		std::cout << "5번 CreateBuffer" << std::endl;
		OutputDebugString(errMsg); // 디버그 출력창에 메시지 표시
...
// 최적 스케일 계산 함수 (예시)
float QDirect3D11Widget::ComputeOptimalScale(double mean, double rms)
{
	// 실제 로직은 구현 필요
	// 여기서는 단순히 평균과 RMS를 조합한 예시
...
ScaleOptimizationStats QDirect3D11Widget::DebugDeltaZTexFull()
{
	...
	//Q3 = 데이터 3사분위수 (상위 25%)
	//IQR = Q3 - Q1
	//IQR 방식은 데이터 분포를 기반으로 극단값(Outlier) 판단

	//lowerBound, upperBound = IQR 기준으로 극단치 제외 범위 결정
	//이 방법은 데이터 분포 기반 => 극단치가 있어도 평균/median에
	//영향 적게 제거 가능


	/*일반적으로 outlier 범위 = [Q1 − kIQR, Q3 + kIQR]

		k = 1.5 → 일반적인 통계 기준

		k = 1.0 → 좀 더 보수적으로 outlier 제거

		범위 밖의 값은 Outlier로 판단하고 제거

		(3) 특징

		데이터 분포 기반 → 평균 / 중앙값에 크게 영향을 주는 극단값만 제거

		데이터가 어떤 형태이든(정규분포, 치우친 분포 등) 적용 가능

		장점 : 유연함, 분포 왜곡 최소

		단점 : 데이터 자체가 매우 좁거나 극단값이 많으면 범위가 넓어짐 
		→ 일부 극단치 남을 수 있음*/

	//IQR 방식의 기원
	//통계학에서 오래전부터 사용된 분포 기반 이상치(outlier) 판단 기법
	
	//신뢰성
	//데이터 분포에 강건함(robust)
	//평균, 표준편차 기반 이상치 판단보다 극단치에 민감하지 않음
	//데이터가 정규분포가 아니어도 사용 가능
	//학계, 금융, 공학, 머신러닝 등 다양한 분야에서 outlier 제거 기준으로
	//광범위하게 사용됨
	...
	//  방법 2: 고정 임계값 추가
	//절대 기준 => 어떤 픽셀도 30mm 이상이면 outlier 처리
	...
	/*int validCount=validDeltas.size() ;
	int totalPixels = desc.Width * desc.Height;
	stats.validRatio = float(validCount) / totalPixels;

	int inlier2{};
	int inlier5{};

	for (float val : validDeltas)
	{
		if (val < 2.0f) ++inlier2;
		if (val < 5.0f) ++inlier5;
	}

	stats.inlier2mm = float(inlier2) / validDeltas.size();
	stats.inlier5mm = float(inlier5) / validDeltas.size();

	stats.isValid =
		(stats.inlier2mm > 0.7f) &&
		(stats.validRatio > 0.5f) &&
		(stats.medianDelta < 3.0f);*/
	...
	//qDebug() << "Inlier <2mm:" << stats.inlier2mm * 100 << "%";
	//qDebug() << "Inlier <5mm:" << stats.inlier5mm * 100 << "%";
	//qDebug() << "Coverage:" << stats.validRatio * 100 << "%";
	//qDebug() << "Verdict:" << (stats.isValid ? "PASS" : "FAIL");
	...
	if (0==nFiltered % 2 )
		stats.medianDelta = (filteredDeltas[nFiltered / 2 - 1] 
			+ filteredDeltas[nFiltered / 2]) / 2.0f;
	else
...
void QDirect3D11Widget::RenderVolumeView()
{
	...
	//m_pDeviceContext->OMSetDepthStencilState(m_depthStencilState, 1);
		...
		ID3D11BlendState* volumeBlend = nullptr;
		m_pDevice->CreateBlendState(&blendDesc, &volumeBlend);
		m_pDeviceContext->OMSetBlendState(volumeBlend, nullptr, 0xffffffff);
		...
		// 1. RTV 설정
		ID3D11RenderTargetView* rtvs[1] = { m_pSwapChainRTV };
...
void QDirect3D11Widget::RenderAllQuads()
{
	...
	// 좌상단으로 이동 (예: (10,10)) + 박스 크기 키우기 (예: 250x150)
...
void QDirect3D11Widget::UpdateSlicePlanePositions() {
	...
	// ------------------------------
	{
		...
		//	}
			...
			// Axial (XY 평면)
		//float axialSize = max(scaleX, scaleY) * overallSize;
		float axialSize{ max(scaleX, scaleY) * 1000 };
