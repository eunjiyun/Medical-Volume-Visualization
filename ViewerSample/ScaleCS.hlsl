...
cbuffer CB : register(b0)
{
	...
	float4 VoxelAndMaxSteps;   // w = maxSteps
	...
	out float deltaZ)
{
	...
	for (int i = 0; i < maxSteps; ++i)
	{
		...
		float3 boxMinL = -volSize.xyz * 0.5;
		float3 boxMaxL = volSize.xyz * 0.5;
		...
		if (hu > 300.0)
		{
			float hitViewZ = mul(float4(posWS, 1), View).z;
...
//numthreads는 한 그룹에 몇 명의 작업자를 넣을지
//이 부분은 compute shader에서 병렬 실행 단위를 어떻게 묶을지 
//지정하는 개념 
[numthreads(1, 1, 1)]
//[numthreads(8, 8, 1)]
void main( uint3 DTid : SV_DispatchThreadID )
{
	...
	// ray → volume local
	float3 rayPosL = mul(float4(rayPosWS, 1), InvVolumeWorld).xyz;
	float3 rayDirL = mul(float4(rayDirWS, 0), InvVolumeWorld).xyz;
	...
	{
		DeltaZTex[pixelCoord] = -1;
		//return float4(0, 0, 0, 1); // or early-out
		return;
	}

	// local → world t
	float3 entryWS = mul(float4(rayPosL + rayDirL * tNearL, 1), VolumeWorld).xyz;
	float3 exitWS = mul(float4(rayPosL + rayDirL * tFarL, 1), VolumeWorld).xyz;

	float tNearW = max(0.0, dot(entryWS - rayPosWS, rayDirWS));
	float tFarW = dot(exitWS - rayPosWS, rayDirWS);


	float deltaZ;
	bool hit = ComputeDeltaZ(
		rayPosWS,
		rayDirWS,
		tNearW,
		tFarW,
	...
	DeltaZTex[pixelCoord] = hit ? deltaZ : -1.0;
}